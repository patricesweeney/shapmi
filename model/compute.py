...
def prefilter_features(df: pd.DataFrame, feature_cols: list[str], target_col: str) -> list[str]:
    kept: list[str] = []
    ...
    for name in feature_cols:
        ...
        if not is_dt and (pd.api.types.is_string_dtype(s) or s.dtype == object):
            ...
            is_dt = float(parsed.notna().sum()) / float(len(s_no_na)) >= 0.8 if len(s_no_na) else False
        if is_dt:
...
def dist_params_for(series: pd.Series):
    params = {}
    s = series.dropna()
...
def analyze_dataset(df: pd.DataFrame, target: str):
    if target not in df.columns:
        raise ValueError(f"Target '{target}' not in DataFrame")

    feature_cols = [c for c in df.columns if c != target]
    feature_cols = prefilter_features(df, feature_cols, target)
    ...
    dist_params = {col: dist_params_for(df[col]) for col in overview_cols}
